...
import { Image as ImageIcon, ArrowLeftRight, Layout, Palette, FileText, Bold, Italic, Underline, AlignLeft, AlignCenter, AlignRight, ChevronDown, ChevronRight } from 'lucide-react';
...
export const RightSidebar = () => {
  ...
  if (!selectedScreen) {
    return (
      <div className="sidebar sidebar-right" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '40px', textAlign: 'center', color: 'var(--text-muted)' }}>
        <div style={{
          width: '64px',
          height: '64px',
          borderRadius: '50%',
  ...
  // Internal helper for redundant typography controls
  const TypographyControls = ({ type }: { type: 'title' | 'subtitle' }) => {
    const isTitle = type === 'title';
    const color = isTitle ? selectedScreen.titleColor : selectedScreen.subtitleColor;
    const size = isTitle ? selectedScreen.titleSize : selectedScreen.subtitleSize;
    const isBold = isTitle ? selectedScreen.titleBold : selectedScreen.subtitleBold;
    const italic = isTitle ? selectedScreen.titleItalic : selectedScreen.subtitleItalic;
    const underline = isTitle ? selectedScreen.titleUnderline : selectedScreen.subtitleUnderline;
    const align = isTitle ? (selectedScreen.titleAlign || 'center') : (selectedScreen.subtitleAlign || 'center');

    const setColor = (val: string) => updateScreen(selectedScreen.id, { [isTitle ? 'titleColor' : 'subtitleColor']: val });
    const setSize = (val: number | undefined) => updateScreen(selectedScreen.id, { [isTitle ? 'titleSize' : 'subtitleSize']: val });
    const setBold = (val: boolean) => updateScreen(selectedScreen.id, { [isTitle ? 'titleBold' : 'subtitleBold']: val });
    const setItalic = (val: boolean) => updateScreen(selectedScreen.id, { [isTitle ? 'titleItalic' : 'subtitleItalic']: val });
    const setUnderline = (val: boolean) => updateScreen(selectedScreen.id, { [isTitle ? 'titleUnderline' : 'subtitleUnderline']: val });
    const setAlign = (val: 'left' | 'center' | 'right') => updateScreen(selectedScreen.id, { [isTitle ? 'titleAlign' : 'subtitleAlign']: val });
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              <div>
                ...
                <input
                  ...
                  max="2"
                  step="0.01"
                  value={selectedScreen.scale || (device === 'iPad' ? 1.7 : 1.0)}
            ...
            <div style={{ marginTop: '8px' }}>
              ...
              <div style={{
                ...
                {expandedSections.includes('subtitle') && selectedScreen.showSubtitle !== false && (
                  <div style={{ padding: '16px' }}>
                    <TypographyControls type="subtitle" />
                  </div>
                )}
              </div>
            </div>
